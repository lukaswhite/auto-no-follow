...
class NoFollowTest extends \PHPUnit\Framework\TestCase
{
     ...
     */
    public function testGetsCurrentHostFromServerGlobalIfNotSetExplictly()
    {
        $processor = new \Lukaswhite\AutoNoFollow\Processor();
        $host = $processor->getCurrentHost();
    ...
    /**
     * @expectedException \Lukaswhite\AutoNoFollow\Exceptions\ParserException
     */
    public function testExceptionThrownIfHTMLCannotBeParsed()
    {
        $processor = new \Lukaswhite\AutoNoFollow\Processor();
        $processor->setCurrentHost('localhost');
        $output = $processor->addLinks('<p></span><html>');
    }
